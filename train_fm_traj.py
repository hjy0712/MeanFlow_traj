"""
训练普通flow matching用于轨迹生成，带no goal的版本
...
def main():
    ...
    os.makedirs("runs_nogoal/images", exist_ok=True)
    os.makedirs("runs_nogoal/checkpoints", exist_ok=True)

    if accelerator.is_main_process:
        writer = SummaryWriter(log_dir="runs_nogoal/tensorboard_logs")
    ...
    with tqdm(range(n_steps), disable=not accelerator.is_main_process, dynamic_ncols=True) as pbar:
        ...
        for step in pbar:
            ...
            for _ in range(repeat_per_batch):
                ...
                # import ipdb; ipdb.set_trace()  # 暂时注释掉调试断点
                loss, mse_val = net.compute_flow_matching_loss(
                    a_start=a_start,
                ...
                if accelerator.is_main_process and global_step % log_step == 0:
                    ...
                    print(log_msg)
                    with open("runs_nogoal/train_log.txt", "a") as f:
                        f.write(log_msg + "\n")
                ...
                if accelerator.is_main_process and global_step % ckpt_step == 0:
                    ckpt_path = f"runs_nogoal/checkpoints/navdpflow_step_{global_step}.pt"
                ...
                if accelerator.is_main_process and global_step % sample_step == 0:
                    ...
                    with torch.no_grad():
                        ...
                        if sample_pic is not None:
                            img_save_path = f"runs_nogoal/images/step_{global_step}.png"
